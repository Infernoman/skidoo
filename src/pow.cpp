...
bool CheckBlockProofOfWork(const CBlockHeader *pblock, int nHeight)
{
	...
	if (pblock->nVersion > 2) {
       LogPrintf("nVersion : %d, ChainID : %d, %d\n",pblock->nVersion,pblock->GetChainID(),(nHeight >= 825000 ? AUXPOW_CHAIN_ID_FORK : AUXPOW_CHAIN_ID));

       if (!Params().AllowMinDifficultyBlocks() && (pblock->nVersion & BLOCK_VERSION_AUXPOW && pblock->GetChainID() != (nHeight >= 825000 ? AUXPOW_CHAIN_ID_FORK : AUXPOW_CHAIN_ID)))
